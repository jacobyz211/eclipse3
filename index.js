...
app.get('/u/:token/artist/:id', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    const userResp = await axios.get(
      ...
      {
        params: { client_id: effectiveCid(entry) },
        headers: { 'User-Agent': UA, Accept: 'application/json' },
        timeout: 12000
      }
    );
    ...
    const genres     = []; // SC doesn't really give clean genres here

    // 2) Fetch artist tracks and compute popularity (plays + likes + reposts)
    const tracksResp = await axios.get(
      'https://api-v2.soundcloud.com/users/' + encodeURIComponent(userId) + '/tracks',
      {
        params: {
          client_id: effectiveCid(entry),
          limit: 50,
    ...
    const scored = rawTracks
      ...
      .map(t => {
        ...
        return {
          ...
          artworkURL: art,
          // Eclipse will call /stream later, so no streamURL here
          streamURL: null,
          format: 'mp3'
        };
      })
    ...
    // 3) Existing albums-from-playlists logic (kept simple here)
    let albums = [];
    try {
    ...
    // 4) Respond with topTracks + albums so Eclipse shows popular songs too
    res.json({
      id: rawId,
      name: artistName,
      artworkURL,
