...
// ─── Monochrome / Hi-Fi instances ─────────────────────────────────────────
// These are community-hosted Hi-Fi API v2.7 instances that proxy TIDAL.
// Used as a fallback when a SoundCloud track is SNIP/BLOCK/unavailable.
...
async function checkInstances() {
  for (var inst of HIFI_INSTANCES) {
    try {
      ...
      activeInstance = inst; instanceHealthy = true;
      console.log('[hifi] healthy: ' + inst); return;
    } catch (e) {}
  }
...
// Search Monochrome for a track by title+artist and return a streamable URL.
// Tries up to 3 query variations for resilience.
async function monochromeSearch(title, artist) {
  var queries = [];
  ...
  for (var q of queries) {
    try {
      var data = await hifiGetSafe('/search/', { s: q, limit: 5, offset: 0 });
      if (!data) continue;
      var items = (data.data && data.data.items) ? data.data.items : [];
      var match = items.find(function (t) { return t && t.id && t.streamReady !== false && t.allowStreaming !== false; });
      if (!match) continue;

      for (var qi = 0; qi < qualities.length; qi++) {
        try {
          var payload = await hifiGet('/track/', { id: String(match.id), quality: qualities[qi] });
          if (payload && payload.manifest) {
            var decoded = decodeManifest(payload.manifest);
            if (decoded && decoded.url) {
              var fmt = (decoded.codec && decoded.codec.toLowerCase().indexOf('flac') !== -1) ? 'flac' : 'aac';
              console.log('[monochrome] found: ' + match.title + ' (' + qualities[qi] + ')');
              return { url: decoded.url, format: fmt, quality: qualities[qi].toLowerCase(), expiresAt: Math.floor(Date.now() / 1000) + 3600 };
            }
          }
        } catch (e) {
          if (qi === qualities.length - 1) { console.warn('[monochrome] all qualities failed for track ' + match.id); break; }
        }
      }
    } catch (e) { console.warn('[monochrome] search failed for "' + q + '": ' + e.message); }
  }
  return null;
}
...
function detectUrlType(url) {
  ...
  if (/on\.soundcloud\.com\/.+/.test(url))          return 'sc_playlist';
  if (/snd\.sc\/.+/.test(url))                       return 'sc_playlist';
  return null;
}
...
function buildConfigPage(baseUrl) {
  ...
  h += '<div class="card">';
  h += '<h1>Eclipse</h1><p class="sub">SoundCloud addon with Monochrome fallback. SNIP &amp; BLOCK tracks automatically fall back to the TIDAL catalog via Hi-Fi API.</p>';
  h += '<div class="pills">';
  h += '<span class="pill">SoundCloud</span>';
  h += '<span class="pill o">Monochrome Fallback</span>';
  h += '<span class="pill b">FLAC / AAC 320</span>';
  h += '</div>';
  h += '<div class="lbl">Optional: Your SoundCloud Client ID</div>';
  h += '<input type="text" id="clientId" placeholder="Leave blank to use shared client_id (auto-refreshed)">';
  h += '<div class="hint">Only needed if the shared ID stops working. <a href="https://soundcloud.com" target="_blank">soundcloud.com</a> &rarr; DevTools &rarr; Network &rarr; find <code>client_id=</code></div>';
  h += '<div class="lbl">Generate New Addon URL</div>';
  h += '<button class="bo" id="genBtn" onclick="generate()">Generate My Addon URL</button>';
  h += '<div class="box" id="genBox"><div class="blbl">Your addon URL</div><div class="burl" id="genUrl"></div>';
  h += '<button class="bd" id="copyGenBtn" onclick="copyGen()">Copy URL</button></div>';
  h += '<hr>';
  h += '<div class="lbl">Refresh Existing URL</div>';
  h += '<input type="text" id="existingUrl" placeholder="Paste your existing addon URL">';
  h += '<button class="bo" id="refBtn" onclick="doRefresh()">Refresh Existing URL</button>';
  h += '<div class="box" id="refBox"><div class="blbl">Refreshed addon URL</div><div class="burl" id="refUrl"></div>';
  h += '<button class="bd" id="copyRefBtn" onclick="copyRef()">Copy URL</button></div>';
  h += '</div>';
  ...
  h += '<div class="step"><div class="sn">2</div><div class="st">Copy the URL and paste it into <b>Eclipse Music &rarr; Settings &rarr; Addons</b></div></div>';
  h += '<div class="step"><div class="sn">3</div><div class="st">Search SoundCloud tracks &mdash; SNIP/BLOCK tracks automatically fall back to <b>Monochrome (TIDAL)</b></div></div>';
  h += '<div class="step"><div class="sn">4</div><div class="st">Import SoundCloud playlists as CSV using the tool below</div></div>';
  h += '</div>';
  h += '<div class="warn">Monochrome instances are community-hosted. The addon auto-discovers healthy instances and fails over automatically.</div>';
  h += '</div>';

  h += '<div class="card">';
  h += '<h2>Playlist Import</h2>';
  h += '<p class="sub" style="margin-bottom:14px">Import a SoundCloud playlist as a CSV for use in Eclipse Music.</p>';
  ...
  h += '<input type="text" id="impUrl" placeholder="soundcloud.com/artist/sets/name  or  on.soundcloud.com/XXXX">';
  h += '<div class="hint">SoundCloud: soundcloud.com/*/sets/* or on.soundcloud.com/... or snd.sc/...</div>';
  ...
  h += '<footer>Eclipse SoundCloud Addon v3.9.0 &bull; Monochrome Fallback &bull; <a href="' + baseUrl + '/health" target="_blank" style="color:#333;text-decoration:none">' + baseUrl + '</a></footer>';
...
app.get('/u/:token/manifest.json', tokenMiddleware, function (req, res) {
  res.json({
    ...
    name:        'SoundCloud',
    version:     '3.9.0',
    description: 'SoundCloud + Monochrome (TIDAL) fallback. SNIP/BLOCK tracks automatically served from Hi-Fi API. Full tracks only.',
...
app.get('/u/:token/search', tokenMiddleware, async function (req, res) {
  ...
  try {
    ...
    res.json({
      ...
      artists:   ((results[2] && results[2].collection) || []).map(function (u) { return { id: String(u.id), name: u.username || 'Unknown', artworkURL: artworkUrl(u.avatar_url) }; })
    });
  } catch (e) { console.error('[search] ' + e.message); res.status(500).json({ error: 'Search failed.', tracks: [], albums: [], artists: [], playlists: [] }); }
});
...
app.get('/u/:token/stream/:id', tokenMiddleware, async function (req, res) {
  ...
  // Attempt SC progressive stream for fully playable tracks.
  // SNIP/BLOCK/unknown policy skip straight to Monochrome fallback.
  ...
  // Monochrome (TIDAL Hi-Fi) fallback for SNIP/BLOCK/unavailable tracks
  ...
  console.log('[stream] falling back to Monochrome for: ' + meta.title);
  var mc = await monochromeSearch(meta.title, meta.artist || meta.uploader || '');
  if (mc) return res.json(mc);
...
app.get('/u/:token/import', tokenMiddleware, async function (req, res) {
  ...
  return res.status(400).json({ error: 'URL not recognised. Use soundcloud.com/*/sets/*, on.soundcloud.com/..., or snd.sc/...' });
});
...
app.get('/health', function (_req, res) {
  res.json({
    ...
    monochromeReady: instanceHealthy,
    monochromeInstance: activeInstance,
    activeTokens: TOKEN_CACHE.size,
    timestamp: new Date().toISOString()
  });
});

app.listen(PORT, function () { console.log('Eclipse SoundCloud Addon v3.9.0 on port ' + PORT); });
