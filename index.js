...
function buildConfigPage(baseUrl) {
  ...
  h += 'var lines=["Track URI\\tTrack Name\\tAlbum Name\\tArtist Name(s)\\tRelease Date\\tDuration (ms)\\tPopularity\\tExplicit\\tAdded By\\tAdded At\\tGenres\\tRecord Label\\tDanceability\\tEnergy\\tKey\\tLoudness\\tMode\\tSpeechiness\\tAcousticness\\tInstrumentalness\\tLiveness\\tValence\\tTempo\\tTime Signature"];function ce(s){return String(s==null?"":s).replace(/[\\t\\r\\n]+/g," ");}tracks.forEach(function(t){var dur=t.duration?Math.round(t.duration*1000):0;lines.push(["",ce(t.title),ce(data.title||""),ce(t.artist||""),"",dur,"0","false","","","","","","","","","","","","","","","",""].join("\\t"));});';
  h += 'var blob=new Blob([lines.join("\\r\\n")],{type:"text/plain;charset=utf-8;"});var a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=(data.title||"playlist").replace(/[^a-zA-Z0-9 \\-_\\.]/g,"").trim()+".csv";document.body.appendChild(a);a.click();document.body.removeChild(a);';
